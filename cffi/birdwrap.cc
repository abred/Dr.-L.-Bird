...
int* Wrapper::histogram(Rectangle r, int width, int height) {
  if (histo == nullptr) {
    histo = new int[512] {0};
  }
  memset(histo, 0, sizeof(int)*512);
  // for (int i = 0; i < 512; i++) {
  //   histo[i] = 0;
  // }
  // int* h = new int[512] {0};
  ...
  for (int y = r.y; y < r.y + r.h; y++) {
    ...
    for (int x = r.x; x < r.x + r.w; x++) {
      if ((x < 0) || (x >= width))
        continue;
      histo[compColors[components[y * width + x]]] += 1;
    }
  }
...
void Wrapper::processScreenShotImpl(unsigned char* input,
                                    int* output,
...
void Wrapper::preprocessDataForNNImpl(int* input,
                                      float* output,
                                      int width, int height) {
  // extract width and height
  if ((height != 480) && (width != 840)) {
...
Point Wrapper::findSlingshotCenterImpl(int* scene,
                                       int width,
                                       int height) {
  Rectangle obj;
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      ...
      if (obj.h > 10) {
        ...
        histCol = histogram(col, width, height);
        histCol511 = histCol[511];
        while (histCol511 >= obj.h * 0.8 && obj.h > 10) {
      ...
      if ((hist345 > std::max(32.0, 0.1 * obj.w * obj.h))
          && (hist64 != 0)) {
...
void Wrapper::findConnectedComponents(int* image,
                                      int width, int height)
{
  int n = 0;
...
void Wrapper::findBoundingBoxes(int* image,
                                int width, int height,
                                int numComp) {
  if (boxes != nullptr) {
    delete[] boxes;
...
int Wrapper::getScoreInGame(int const* screenshot, int* output, int width, int height) {
  // crop score image
  int subImgH = 32;
  int subImgW = 200;
  int startX = 632;
...
int Wrapper::countToDigitInGame(int count)
{
  int digit = 0;
  switch (count) {
...
int Wrapper::getScoreEndGame(unsigned char const* screenshot, int width, int height,
                    int threshold) {
  // crop score image
  int subImgH = 32;
  int subImgW = 100;
...
std::list<Rectangle> Wrapper::findRedBirdsMBRs() {
...
std::list<Rectangle> Wrapper::findBlueBirdsMBRs() {
  std::list<Rectangle> objects;
...
std::list<Rectangle> Wrapper::findYellowBirdsMBRs() {
  std::list<Rectangle> objects;
...
std::list<Rectangle> Wrapper::findWhiteBirdsMBRs() {
  std::list<Rectangle> objects;
  ...
  for (int n = 0; n < numComponents; n++) {
    ...
    int* hist = histogram(obj, nWidth, nHeight);
    int hist510 = hist[510];
    int hist508 = hist[508];
    if (hist510 > 0 && hist508 > 0) {
      objects.push_back(obj);
    }
    // delete[] hist;
  }
...
std::list<Rectangle> Wrapper::findBlackBirdsMBRs() {
  std::list<Rectangle> objects;
  ...
  for (int n = 0; n < numComponents; n++) {
    ...
    int hist0 = hist[0];
    int hist64 = hist[64];
    int hist385 = hist[385];
    if ((hist0 > std::max(32.0, 0.1 * obj.w * obj.h))&& hist64 > 0 && hist385 == 0) {
      objects.push_back(obj);
    }
    // delete[] hist;
  }
